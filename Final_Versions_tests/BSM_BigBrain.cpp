...
inline void 
gaussian_box_muller_4(std::vector<double> &Z_v, ui64 num_simulations, ui64 global_seed)
{
...
double 
BSMC(const double sq_and_sigma, const double k, const double res
        , const ui64 num_simulations, const ui64 global_seed)
{
    double sum_payoffs = 0.0;
    double ST = 0.0;

    std::vector<double> Z_v(num_simulations);
    logNorm(Z_v, num_simulations, global_seed);

    for(ui64 i = 0; i < num_simulations; i++)
    {
        ST          = Z_v[i] * sq_and_sigma;
        results[i]  = fastexp(ST);
    }
...
int main(int argc, char* argv[]) {
    ...
    for (ui64 run = 0; run < num_runs; ++run) {
        ...
        sum+= BSMC(k, res, num_simulations, sq_and_sigma, global_seed);
    }
