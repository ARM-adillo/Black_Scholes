...
inline void gaussian_box_muller(std::vector<double> &Z_v, ui64 num_simulations, ui64 global_seed)
{
...
double black_scholes_monte_carlo(std::vector<double> &results, const ui64 S0, const ui64 K, const double drift,
        const double diffusion, const double res, const ui64 num_simulations, ui64 global_seed)
{
    ...
    gaussian_box_muller(Z_v, num_simulations, global_seed);
...
/**********************************************************************************************************************************************/
    ...
    for (ui64 run = 0; run < num_runs; ++run) {
        sum+= black_scholes_monte_carlo(results, S0, K, drift, diffusion, res_, num_simulations, global_seed);
    }
